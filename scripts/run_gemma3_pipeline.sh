#!/bin/bash
# Gemma 3 27B R1: End-to-end experiment pipeline
# Run on RunPod with A40 GPU
set -e

echo "=== Phase 1: Activation Extraction (reuse QwQ text) ==="

# 1a. Copy QwQ generation JSONs to Gemma output dirs
echo "--- Copying QwQ generation files ---"
for concept in trees library currency; do
    case $concept in
        trees) SRC=/workspace/probe_data_v2 DEST=/workspace/probe_data_gemma3_trees ;;
        library) SRC=/workspace/probe_data_library DEST=/workspace/probe_data_gemma3_library ;;
        currency) SRC=/workspace/probe_data_currency DEST=/workspace/probe_data_gemma3_currency ;;
    esac
...
# 1b. Re-extract activations through Gemma (forward pass only, no generation)
echo ""
echo "--- Re-extracting activations through Gemma 3 ---"
for concept in trees library currency; do
    CONFIG="configs/experiments/qa_probe_training_gemma3_${concept}.yaml"
    echo ""
    echo "--- $concept ---"
...
uv run python scripts/extract_prewritten_activations.py --config configs/experiments/deception_prewritten_gemma3.yaml
...
uv run python scripts/train_probes_qa.py --config configs/experiments/deception_prewritten_gemma3.yaml
...
uv run python scripts/evaluate_probes_qa.py --config configs/experiments/deception_prewritten_gemma3.yaml

echo ""
echo "=== Phase 4: Evasion Experiments ==="
for concept_cfg in evasion_affordances_gemma3 evasion_library_gemma3 evasion_currency_gemma3; do
    echo ""
    echo "--- $concept_cfg ---"
...
GCG_CONFIG="configs/experiments/deception_neutral_gemma3.yaml"
GCG_PROBE_DIR="/workspace/deception_prewritten_data_gemma3/probes"
GCG_TRACES="data/concepts/deception_neutral/traces_handcrafted.yaml"
GCG_OUTPUT="/workspace/deception_neutral_data_gemma3/gcg"
...
BPJ_OUTPUT="/workspace/deception_neutral_data_gemma3/bpj"
