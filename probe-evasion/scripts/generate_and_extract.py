...
2. Generate full response with model.generate()
3. Find token positions (last_token, end_of_reasoning, first_answer_sentence_end, answer range)
4. Run one forward pass on the full sequence to extract activations at all positions
...
def process_single_prompt(
    ...
    """
    Generate response and extract multi-position activations for a single prompt.
    ...
    # Format as chat prompt — constrain answer to 1-3 sentences to keep
    # concept signal concentrated (reasoning trace is still unconstrained)
    ...
    save_generation(
        ...
        positions, num_generated_tokens, total_time, output_dir,
    )
...
def main():
    ...
    # Process prompts
    print("\n" + "=" * 60)
    print("Step 3: Generating and extracting")
    print("=" * 60)
    print(f"  Target layers: {target_layers}")
    print(f"  Token positions: {config['token_positions']}")
    print(f"  Generation: max_new_tokens={generation_config.get('max_new_tokens', 4096)}, "
    ...
    for i, prompt_info in enumerate(remaining):
        prompt_id = prompt_info["prompt_id"]
        label = prompt_info["label"]
        print(f"\n  [{i+1}/{len(remaining)}] {prompt_id} ({label})")
        print(f"    Q: {prompt_info['question'][:80]}...")
        ...
        try:
            ...
            # Append to log (atomic: write then flush)
            ...
            eta_hours = (len(remaining) - i - 1) / rate if rate > 0 else 0

            print(f"    Generated {log_entry['num_generated_tokens']} tokens "
                  f"(think={log_entry['thinking_tokens']}, ans={log_entry['answer_tokens']})")
            print(f"    Time: gen={log_entry['gen_time']:.1f}s, "
                  f"extract={log_entry['extract_time']:.1f}s, "
                  f"total={log_entry['total_time']:.1f}s")
            print(f"    Progress: {rate:.0f} prompts/hr, ETA: {eta_hours:.1f}h")
    ...
    print("DONE")
    print("=" * 60)
    print(f"  Processed: {len(remaining)} prompts")
